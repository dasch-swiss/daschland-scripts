...
def main():
    ...
    for _, row in character_df.iterrows():
        ...
        resource = excel2xml.make_resource(
            label=resource_label, restype=":Character", id=resource_id
        )
        ...
        if names:
            resource.append(
                excel2xml.make_text_prop(":hasName", names)
            )
        ...
        if excel2xml.check_notna(row["Colour"]):
            ...
            resource.append(
                excel2xml.make_color_prop(":hasColour", color)
            )
        if excel2xml.check_notna(row["Role List"]):
            roles = [x.strip() for x in row["Role List"].split(",")]
            resource.append(
                excel2xml.make_list_prop("Role", ":hasRoleList", roles)
            )
        if excel2xml.check_notna(row["Quote"]):
            resource.append(
                excel2xml.make_text_prop(
                    ":hasQuote", excel2xml.PropertyElement(
                        row["Quote"],
                        encoding="xml"
                    )
                )
        ...
        if excel2xml.check_notna(row["Keyword"]):
            ...
            for keyword in keywords_names_raw:
                if keyword not in keyword_labels_to_names:
                    print(
                        f"Keyword {keyword} - {resource_label} not found in the json file."
                    )
                    continue
