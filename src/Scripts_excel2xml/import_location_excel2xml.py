    ...
    for _, row in location_df.iterrows():
        ...
        if row["Location Type List"] == "Real World":
            resource = excel2xml.make_resource(
                label=resource_label, restype=":LocationRealWorld", id=resource_id
            )
        elif row["Location Type List"] == "Wonderland":
            resource = excel2xml.make_resource(
                label=resource_label, restype=":LocationWonderland", id=resource_id
            )
        else:
            resource = excel2xml.make_resource(
                label=resource_label, restype=":Location", id=resource_id
            )
        ...
        if descriptions:
            ...
            resource.append(
                excel2xml.make_text_prop(":hasDescription", text_elements)
            )
        ...
        if excel2xml.check_notna(row["Geoname ID"]):
            resource.append(
                excel2xml.make_geoname_prop(":hasGeoname", row["Geoname ID"])
            )
