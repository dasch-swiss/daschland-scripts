...
def main():
    ...
    for _, row in image_df.iterrows():
        ...
        resource = excel2xml.make_resource(
            label=resource_label, restype=":Image", id=resource_id
        )

        # add file to resource
        resource.append(
            excel2xml.make_bitstream_prop(image_path, permissions=row["Permission"])
        )
        ...
        if excel2xml.check_notna(row["Book ID"]):
            ...
            resource.append(
                excel2xml.make_resptr_prop(":isPartOfBook", book_id)
            )
        if excel2xml.check_notna(row["Seqnum"]):
            resource.append(
                excel2xml.make_integer_prop(":hasSeqnum", row["Seqnum"])
            )
