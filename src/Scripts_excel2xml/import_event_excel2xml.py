    ...
    for _, row in event_df.iterrows():
        ...
        if row["Event Type"] == "Social":
            resource = excel2xml.make_resource(
                label=resource_label, restype=":EventSocial", id=resource_id
            )
        elif row["Event Type"] == "Conflict":
            resource = excel2xml.make_resource(
                label=resource_label, restype=":EventConflict", id=resource_id
            )
        elif row["Event Type"] == "Adventure":
            resource = excel2xml.make_resource(
                label=resource_label, restype=":EventAdventure", id=resource_id
            )
        ...
        if excel2xml.check_notna(row["Image ID"]):
            ...
            resource.append(
                excel2xml.make_resptr_prop(":linkToImage", image_ids)
            )
        ...
        if excel2xml.check_notna(row["Guest ID"]):
            ...
            resource.append(
                excel2xml.make_resptr_prop(":linkToCharacter", guest_ids)
            )
        if excel2xml.check_notna(row["Guest Amount"]):
            resource.append(
                excel2xml.make_integer_prop(":hasGuestAmount", row["Guest Amount"])
            )
        ...
        if excel2xml.check_notna(row["Protagonist ID"]):
            ...
            resource.append(
                excel2xml.make_resptr_prop(
                    ":linkToCharacterProtagonist", protagonist_ids
                )
            )
        if excel2xml.check_notna(row["Antagonist ID"]):
            antagonist_ids = [x.strip() for x in row["Antagonist ID"].split(",")]
            resource.append(
                excel2xml.make_resptr_prop(":linkToCharacterAntagonist", antagonist_ids)
            )

        # append properties for adventure event:
        if excel2xml.check_notna(row["Adventure Character ID"]):
            adventure_character_ids = [
                x.strip() for x in row["Adventure Character ID"].split(",")
            ]
            resource.append(
                excel2xml.make_resptr_prop(":linkToCharacter", adventure_character_ids)
            )
        if excel2xml.check_notna(row["Dangerous"]):
            resource.append(
                excel2xml.make_boolean_prop(":isDangerous", row["Dangerous"])
            )
