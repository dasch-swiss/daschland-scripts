...
def transform_to_daschland(nodegoat_data):
    ...
    for type_name, type_data in nodegoat_data["data"]["types"].items():
        if (
            type_name == "Book"
        ):  # Example for Book, add similar transformations for other types
            transformed_data["book"] = {
                "name": type_data["type"]["name"],
                "description": type_data["object_descriptions"]["Description"][
                    "object_description_name"
                ],
                "author": type_data["object_descriptions"]["Author"][
                    "object_description_name"
                ],
                "publication_date": type_data["object_descriptions"][
                    "Publication Date"
